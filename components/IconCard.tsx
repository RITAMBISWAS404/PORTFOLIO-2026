...
export default function IconCard({ label, right, body, bg, delay = 0, iconColor = "#ffffff", textColor = "#ffffff", border, noHover = false }: Props) {
  ...
  return (
    <div ref={ref}
      ...
      style={{
        position: "relative", background: `var(--exp-card-bg, ${bg})`, border: border ?? "none", borderRadius: 12, padding: 16,
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", position: "relative", zIndex: 1 }}>
        <span className="card-colored-heading" style={{ fontWeight: 600, color: `var(--exp-card-heading, ${textColor})` }}>{label}</span>
        {right && (
          <span className="card-colored-heading" style={{ fontWeight: 600, color: `var(--exp-card-heading, ${textColor})`, opacity: 0.75, flexShrink: 0, marginLeft: 8 }}>
            {right}
          </span>
        )}
      </div>

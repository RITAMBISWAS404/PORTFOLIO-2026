...
import { revealStyle } from "@/lib/experiment/tokensV2";

interface Props { label: string; body: string; delay?: number; icon: IconType; noHover?: boolean; }

// Figma: https://www.figma.com/design/bNFl0RkpGoTcFlRArpiUeI/portfolio-recreate?node-id=7-1179
// White card, no stroke, subtle shadow, icon + title row, description below.
export default function Card({ label, body, delay = 0, icon: Icon, noHover = false }: Props) {
  ...
  return (
    <div ref={ref}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <Icon className="card-icon" color="#222222" />
        <span className="card-heading" style={{ fontWeight: 600, color: "#222222" }}>{label}</span>
      </div>

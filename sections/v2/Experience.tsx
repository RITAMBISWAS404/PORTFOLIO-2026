  ...
  return (
    ...
    onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.background = ""; }}>
      <div style={{ ...col, padding: "16px 0", borderTop: isFirst ? "none" : `1px solid ${C.border}` }}>
          ...
          <div style={{ display:"flex", alignItems:"center", flexWrap:"wrap", gap:8 }}>
            ...
            {e.img
              ? (Array.isArray(e.img) ? e.img : [e.img]).map((src: string, idx: number) => (
                  <div key={idx} style={{ width:24, height:24, borderRadius:12, flexShrink:0, overflow:"hidden" }}>
                    <img src={src} alt={e.company} style={{width:"100%",height:"100%",objectFit:"cover",display:"block"}}/>
                  </div>
                ))
              : <div style={{ width:24, height:24, borderRadius:12, flexShrink:0, background:e.logoBg,
        ...
        <div className="exp-meta-mobile" style={{ fontSize:12, fontWeight:600, color:C.t3, marginTop:4 }}>{fullMeta}</div>
        {/* Desktop: mode + location below */}
        <div className="exp-details-desktop" style={{ fontSize:12, fontWeight:600, color:C.t3, marginTop:4 }}>{details}</div>
      </div>
    </div>
  );
}

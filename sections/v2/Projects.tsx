...
function ProjectCard({p,delay}:{p:typeof projects[0];delay:number}){
  ...
  return(
    <>
      <a ref={ref} href={p.href} target="_blank" rel="noopener noreferrer" style={{border:`1px solid ${C.border}`,borderRadius:12,overflow:"hidden",
        ...
        <div style={{padding:16,display:"flex",flexWrap:"wrap",gap:8}}>
          {p.tags.map(t=>(
            <div key={t} style={{ ...tagStyle, borderRadius: 12 }} onMouseEnter={e=>tagHv(e,true)} onMouseLeave={e=>tagHv(e,false)}>
              {tagIcons[t]}{t}
            </div>
          ))}
        </div>

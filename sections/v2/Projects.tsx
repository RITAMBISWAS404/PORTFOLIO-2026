...
function ProjectCard({p,delay}:{p:typeof projects[0];delay:number}){
  ...
  return(
    <>
      <a ref={ref} href={p.href} target="_blank" rel="noopener noreferrer" style={{border:`1px solid ${C.border}`,borderRadius:20,overflow:"hidden",
        ...
        <div style={{padding:"16px 16px 0"}}>
          <div className="f16" style={{fontWeight:600,color:C.t1}}>{p.title}</div>
          <div className="f16" style={{fontWeight:500,color:C.t2,marginTop:4}}>{p.sub}</div>
        </div>
        <div style={{padding:16,display:"flex",flexWrap:"wrap",gap:8}}>
          {p.tags.map(t=>(
            <div key={t} style={{ ...tagStyle, borderRadius: 20 }} onMouseEnter={e=>tagHv(e,true)} onMouseLeave={e=>tagHv(e,false)}>
              {tagIcons[t]}{t}
            </div>
          ))}
        </div>

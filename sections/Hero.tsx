  ...
  return (
    <div ref={ref} onMouseMove={move} onMouseLeave={leave}
      ...
      <motion.div style={{width:64,height:64,borderRadius:16,overflow:"hidden",transform,
        background:"conic-gradient(from 71deg,#34a853,#4285f4,#fbbc05,#f43f5e,#34a853)"}}>
        <div style={{position:"absolute",inset:0,
          background:"conic-gradient(from 71deg,#34a853,#4285f4,#fbbc05,#f43f5e,#34a853)",
          filter:"blur(8px)",opacity:1,animation:"aura 8s linear infinite"}}/>
...
export default function Hero() {
  ...
  return (
    <section id="hero" style={{maxWidth:720,margin:"0 auto",padding:"32px 24px 0"}}>
      ...
      <style>{`@keyframes aura{from{transform:scale(2.2) rotate(0deg)}to{transform:scale(2.2) rotate(360deg)}}`}</style>
    </section>
  );
}
